...
import{getAgents,deleteAgent,agentAdd} from "../JS/actions"
  ...
  <tbody>
     ...
     {agents.map((obj,i)=>{
        ...
        <tr  key={i}>
         ...
         <td>{obj.email}</td>    
         <td>{obj.post}</td>
         <td><Button variant="danger" style={{width:"100px"}} onClick={()=>{dispatch(deleteAgent({email:obj.email}))}}>delete</Button></td>
         </tr>
         )
       })
      }
